...
if SLM.State == 1
    ...
    if SLM.is_onek && (constructed_okay.value ~= 1)  %apparently now we dont follow C convention
        disp('Blink SDK was not successfully constructed');
        disp(calllib('Blink_C_wrapper', 'Get_last_error_message'));
        calllib('Blink_C_wrapper', 'Delete_SDK');
    elseif SLM.is_onek==0 && (constructed_okay.value ~= 0)% Convention follows that of C function return values: 0 is success, nonzero integer is an error
        disp('Blink SDK was not successfully constructed');
        disp(calllib('Blink_C_wrapper', 'Get_last_error_message'));
        calllib('Blink_C_wrapper', 'Delete_SDK');
    else
